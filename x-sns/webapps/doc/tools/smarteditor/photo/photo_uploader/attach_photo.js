    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/comm/fileupload/fileUploadForSmartEditor.upload'; 	//upload URL
...
//		});
//    	alert('ab');
//		oAjax.header("Content-Type","multipart/form-data; boundary=----WebKitFormBoundary7uIEu9ax8IY8nCde");
...
//    }
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		xhr.onload = function () {
		   ...
		   if (state == xhr.DONE && responseCode == 200) {
                var responseData = this.responseText;
				//성공 시에  responseText를 가지고 array로 만드는 부분.
				makeArrayFromString(responseData);
            }
			
//			var responseString = e._response.responseText;
//			if (e.readyState() == 4) {
//				if (responseString.indexOf("NOTALLOW_") > -1){
//					var sFileName = responseString.replace("NOTALLOW_", "");
//					alert("이미지 파일(jpg,gif,png,bmp)만 업로드 하실 수 있습니다. ("+sFileName+")");
//				} else {
//					//성공 시에  responseText를 가지고 array로 만드는 부분.
//					makeArrayFromString(responseString);
//				}
//			}		    
		};
